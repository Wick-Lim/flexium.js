...
Use `use()` for all state management needs in Flexium:
...
```tsx
import { untrack,use } from 'flexium/core'
...
1. Replacing React's `useState` with Flexium's `use()`
2. Replacing React's `useEffect` with Flexium's `use()`
