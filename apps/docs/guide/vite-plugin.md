...
import { signal, effect, computed } from 'flexium';
...
function Counter() {
  const count = signal(0); // No import needed!
  // ...
}
```
...
3. Check that you're using supported primitives (signal, effect, computed, etc.)
