...
${cyan}${bright}
  ╔═══════════════════════════════════════╗
  ║                                       ║
  ║   ${magenta}✨ Create Flexism App ✨${cyan}           ║
  ║                                       ║
  ║   ${reset}Realtime-first Fullstack Framework  ${cyan}║
  ║                                       ║
  ╚═══════════════════════════════════════╝
${reset}
`)
...
async function getProjectName(rl, args) {
  ...
  if (!projectName) {
    projectName = 'my-flexism-app'
  }

  const validation = validateProjectName(projectName)
  if (!validation.valid) {
    console.log(`${red}❌ ${validation.error}${reset}`)
    console.log(`${yellow}💡 Examples: my-app, my-project, chat-app${reset}\n`)
...
${bright}Commands:${reset}
  ${green}npm run dev${reset}      Start development server
  ${green}npm run build${reset}    Build for production
  ${green}npm run start${reset}    Start production server
