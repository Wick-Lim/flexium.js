...
export function createButton(props: ButtonProps): {
  ...
  if (ariaExpanded !== undefined) button.setAttribute('aria-expanded', String(ariaExpanded));
  ...
  const disabledSignal = typeof disabled === 'boolean' ? signal(disabled) : disabled;
  ...
  function update(newProps: Partial<ButtonProps>): void {
    ...
    if (newProps.disabled !== undefined) {
      const newDisabled = typeof newProps.disabled === 'boolean'
    ...
    if (newProps.loading !== undefined) {
      const newLoading = typeof newProps.loading === 'boolean'
