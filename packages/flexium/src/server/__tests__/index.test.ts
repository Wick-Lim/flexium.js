...
describe('renderToString', () => {
  describe('Basic Elements', () => {
    it('should render a simple div element', () => {
      const fnode = createFNode('div', {}, [])
      expect(renderToString(fnode)).toBe('<div></div>')
    })

    it('should render element with text content', () => {
      const fnode = createFNode('div', {}, ['Hello World'])
      expect(renderToString(fnode)).toBe('<div>Hello World</div>')
    })
    ...
    it('should render nested elements', () => {
      ...
      const fnode = createFNode('div', {}, [child])
      expect(renderToString(fnode)).toBe('<div><span>inner</span></div>')
    })
    ...
    it('should render deeply nested elements', () => {
      ...
      expect(renderToString(outer)).toBe(
  ...
  describe('Void Elements', () => {
    it('should render img as self-closing', () => {
      const fnode = createFNode('img', { src: 'test.jpg', alt: 'test' }, [])
      expect(renderToString(fnode)).toBe('<img src="test.jpg" alt="test"/>')
    })

    it('should render br as self-closing', () => {
      const fnode = createFNode('br', {}, [])
      expect(renderToString(fnode)).toBe('<br/>')
    })

    it('should render input as self-closing', () => {
      const fnode = createFNode('input', { type: 'text', value: 'test' }, [])
      expect(renderToString(fnode)).toBe('<input type="text" value="test"/>')
    })

    it('should render hr as self-closing', () => {
      const fnode = createFNode('hr', {}, [])
      expect(renderToString(fnode)).toBe('<hr/>')
    })

    it('should render meta as self-closing', () => {
      const fnode = createFNode('meta', { charset: 'utf-8' }, [])
      expect(renderToString(fnode)).toBe('<meta charset="utf-8"/>')
    })
    ...
    it('should render link as self-closing', () => {
      ...
      expect(renderToString(fnode)).toBe(
        '<link rel="stylesheet" href="style.css"/>'
      )
    })

    it('should ignore children for void elements', () => {
      const fnode = createFNode('img', { src: 'test.jpg' }, ['ignored'])
      expect(renderToString(fnode)).toBe('<img src="test.jpg"/>')
    })
  })

  describe('HTML Escaping for XSS Prevention', () => {
    it('should escape < and > in text content', () => {
      const fnode = createFNode('div', {}, ['<script>alert("xss")</script>'])
      expect(renderToString(fnode)).toBe(
    ...
    it('should escape & in text content', () => {
      const fnode = createFNode('div', {}, ['Fish & Chips'])
      expect(renderToString(fnode)).toBe('<div>Fish &amp; Chips</div>')
    })

    it('should escape quotes in text content', () => {
      const fnode = createFNode('div', {}, ['He said "Hello"'])
      expect(renderToString(fnode)).toBe('<div>He said &quot;Hello&quot;</div>')
    })

    it('should escape single quotes in text content', () => {
      const fnode = createFNode('div', {}, ["It's a test"])
      expect(renderToString(fnode)).toBe('<div>It&#039;s a test</div>')
    })

    it('should escape attribute values', () => {
      const fnode = createFNode('div', { title: '<script>' }, [])
      expect(renderToString(fnode)).toBe('<div title="&lt;script&gt;"></div>')
    })
  ...
  describe('Signal Value Extraction', () => {
    it('should extract signal value during SSR', () => {
      ...
      const fnode = createFNode('div', {}, [count])
      expect(renderToString(fnode)).toBe('<div>42</div>')
    })

    it('should extract computed signal value', () => {
      const count = signal(5)
      const doubled = computed(() => count.value * 2)
      const fnode = createFNode('div', {}, [doubled])
      expect(renderToString(fnode)).toBe('<div>10</div>')
    })

    it('should handle signal with string value', () => {
      const text = signal('Hello Signal')
      const fnode = createFNode('div', {}, [text])
      expect(renderToString(fnode)).toBe('<div>Hello Signal</div>')
    })

    it('should handle multiple signals as children', () => {
      const first = signal('Hello')
      const second = signal('World')
      const fnode = createFNode('div', {}, [first, ' ', second])
      expect(renderToString(fnode)).toBe('<div>Hello World</div>')
    })
    ...
    it('should extract signal with null value', () => {
      ...
      const fnode = createFNode('div', {}, [empty])
      expect(renderToString(fnode)).toBe('<div></div>')
    })
  ...
  describe('Style Object to CSS String Conversion', () => {
    ...
    it('should handle multiple style properties', () => {
      ...
      const result = renderToString(fnode)
    ...
    it('should escape style values', () => {
      const fnode = createFNode('div', { style: { content: '"<script>"' } }, [])
      expect(renderToString(fnode)).toContain('&quot;&lt;script&gt;&quot;')
    })
  })
  ...
  describe('Nested Elements and Children', () => {
    ...
    it('should handle mixed text and element children', () => {
      ...
      expect(renderToString(fnode)).toBe(
        '<div>Text before <span>bold</span> text after</div>'
      )
    })
    ...
    it('should render complex nested structures', () => {
      ...
      expect(renderToString(article)).toBe(
        '<article><img src="avatar.jpg" alt="Avatar"/><h2>Title</h2><p>Description</p></article>'
      )
    })
  })
  ...
  describe('Function Components', () => {
    it('should render function component', () => {
      ...
      const fnode = createFNode(Component, {}, [])
      expect(renderToString(fnode)).toBe('<div>Hello</div>')
    })
    ...
    it('should pass props to function component', () => {
      ...
      const fnode = createFNode(Component, { name: 'World' }, [])
      expect(renderToString(fnode)).toBe('<div>Hello World</div>')
    })

    it('should handle nested function components', () => {
      const Inner = (props: { text: string }) =>
        createFNode('span', {}, [props.text])
      const Outer = () =>
        createFNode('div', {}, [createFNode(Inner, { text: 'nested' }, [])])
      const fnode = createFNode(Outer, {}, [])
      expect(renderToString(fnode)).toBe('<div><span>nested</span></div>')
    })

    it('should handle component returning text', () => {
      const Component = () => 'Plain text'
      const fnode = createFNode(Component, {}, [])
      expect(renderToString(fnode)).toBe('Plain text')
    })
    ...
    it('should handle component with complex logic', () => {
      ...
      const fnode = createFNode(Component, { items: ['a', 'b', 'c'] }, [])
      expect(renderToString(fnode)).toBe(
  ...
  describe('Null/Undefined/Boolean Handling', () => {
    ...
    it('should handle null in children array', () => {
      const fnode = createFNode('div', {}, ['before', null, 'after'])
      expect(renderToString(fnode)).toBe('<div>beforeafter</div>')
    })

    it('should handle undefined in children array', () => {
      const fnode = createFNode('div', {}, ['before', undefined, 'after'])
      expect(renderToString(fnode)).toBe('<div>beforeafter</div>')
    })

    it('should handle false in children array', () => {
      const fnode = createFNode('div', {}, ['before', false, 'after'])
      expect(renderToString(fnode)).toBe('<div>beforeafter</div>')
    })
    ...
    it('should handle mixed null/undefined/false children', () => {
      ...
      expect(renderToString(fnode)).toBe('<div>textmore</div>')
    })
  })

  describe('Arrays as Children', () => {
    it('should render array of text nodes', () => {
      const fnode = createFNode('div', {}, [['a', 'b', 'c']])
      expect(renderToString(fnode)).toBe('<div>abc</div>')
    })
    ...
    it('should render array of elements', () => {
      ...
      const fnode = createFNode('div', {}, [children])
      expect(renderToString(fnode)).toBe('<div><p>1</p><p>2</p><p>3</p></div>')
    })

    it('should handle nested arrays', () => {
      const fnode = createFNode('div', {}, [[['nested', 'array']]])
      expect(renderToString(fnode)).toBe('<div>nestedarray</div>')
    })
    ...
    it('should flatten mixed arrays and elements', () => {
      ...
      const fnode = createFNode('div', {}, [['a', 'b'], span, ['c', 'd']])
      expect(renderToString(fnode)).toBe('<div>ab<span>span</span>cd</div>')
    })
  })

  describe('Class and ClassName Handling', () => {
    it('should render className attribute as class', () => {
      const fnode = createFNode('div', { className: 'container' }, [])
      expect(renderToString(fnode)).toBe('<div class="container"></div>')
    })

    it('should render class attribute', () => {
      const fnode = createFNode('div', { class: 'wrapper' }, [])
      expect(renderToString(fnode)).toBe('<div class="wrapper"></div>')
    })
    ...
    it('should escape class values', () => {
      const fnode = createFNode('div', { className: 'test"><script>' }, [])
      expect(renderToString(fnode)).toBe(
        '<div class="test&quot;&gt;&lt;script&gt;"></div>'
      )
    })
  })
  ...
  describe('Attribute Rendering', () => {
    it('should render string attributes', () => {
      const fnode = createFNode('div', { id: 'main', 'data-test': 'value' }, [])
      const result = renderToString(fnode)
    ...
    it('should skip null attributes', () => {
      const fnode = createFNode('div', { id: 'test', title: null }, [])
      const result = renderToString(fnode)
    ...
    it('should skip undefined attributes', () => {
      const fnode = createFNode('div', { id: 'test', title: undefined }, [])
      const result = renderToString(fnode)
      expect(result).toContain('id="test"')
      expect(result).not.toContain('title')
    })
    ...
    it('should skip false attributes', () => {
      const fnode = createFNode('div', { id: 'test', hidden: false }, [])
      const result = renderToString(fnode)
      expect(result).toContain('id="test"')
    ...
    it('should skip children prop', () => {
      const fnode = createFNode(
        'div',
        { id: 'test', children: 'should be ignored' },
        ['actual child']
      )
      const result = renderToString(fnode)
      expect(result).toContain('id="test"')
    ...
    it('should handle aria- attributes', () => {
      const fnode = createFNode(
        'button',
        { 'aria-label': 'Close', 'aria-hidden': 'false' },
        []
      )
      const result = renderToString(fnode)
  ...
  describe('Complex Real-World Scenarios', () => {
    ...
    it('should render a form with inputs', () => {
      const nameInput = createFNode(
        'input',
        { type: 'text', name: 'name', placeholder: 'Name' },
        []
      )
      const emailInput = createFNode(
        'input',
        { type: 'email', name: 'email', placeholder: 'Email' },
        []
      )
      const submitBtn = createFNode('button', { type: 'submit' }, ['Submit'])
      const form = createFNode('form', { method: 'POST', action: '/submit' }, [
        nameInput,
        emailInput,
        submitBtn,
      ])
      ...
      expect(result).toBe(
        '<form method="POST" action="/submit"><input type="text" name="name" placeholder="Name"/><input type="email" name="email" placeholder="Email"/><button type="submit">Submit</button></form>'
      )
    })
    ...
    it('should render a navigation menu', () => {
      const links = ['Home', 'About', 'Contact'].map((text) =>
        createFNode('li', {}, [
          createFNode('a', { href: `/${text.toLowerCase()}` }, [text]),
        ])
      )
    ...
    it('should handle conditional rendering with signals', () => {
      ...
      const fnode = createFNode(Component, {}, [])
      expect(renderToString(fnode)).toBe('<div>Visible content</div>')
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle number zero as child', () => {
      const fnode = createFNode('div', {}, [0])
      expect(renderToString(fnode)).toBe('<div>0</div>')
    })

    it('should handle negative numbers', () => {
      const fnode = createFNode('div', {}, [-42])
      expect(renderToString(fnode)).toBe('<div>-42</div>')
    })

    it('should handle very long text content', () => {
      const longText = 'a'.repeat(10000)
      const fnode = createFNode('div', {}, [longText])
      const result = renderToString(fnode)
