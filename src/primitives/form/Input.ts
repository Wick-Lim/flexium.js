...
export function createInput(props: InputProps): {
  ...
  if (value) {
    ...
    if (onChange) {
      ...
      input.addEventListener('change', handleChangeEvent);
      disposers.push(() => input.removeEventListener('change', handleChangeEvent));
    }
  }
  ...
  function update(newProps: Partial<InputProps>): void {
    ...
    if (newProps.placeholder !== undefined) input.placeholder = newProps.placeholder;
 ...
 */
export function createInputField(props: InputProps & {
  label?: string;
  helperText?: string;
  showError?: boolean;
}): {
  ...
  const { label, helperText, error, showError = true, id: providedId, ...inputProps } = props;
