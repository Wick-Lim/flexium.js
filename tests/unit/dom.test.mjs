...
import { h, Fragment, render, DOMRenderer } from '../../packages/flexium/dist/dom.mjs';
...
test('render() handles padding props', () => {
  ...
  const element = container.children[0];
  assert.strictEqual(element.style.paddingTop, '20px');
...
test('render() handles margin props', () => {
  ...
  const element = container.children[0];
  assert.strictEqual(element.style.marginBottom, '15px');
